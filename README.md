...
- Implements a multi-dimensional, fully customizable, tag-based classification system.
- Pushes and pulls emails from Mail.app, takes over text with PopClip, and connects with Contacts.app.
- Simple to configure, use, and adapt. Behavior is controlled at database level.
...
- Auto-extraction of document amounts for defined dimension values.
- Custom Metadata can be enriched directly through PopClip.
...
- Classification data can be changed at any time; dependent fields reflect the changes.
- Optional contact/sender processing and smart-group navigation for dimension values and DEVONthink labels.
...
- macOS
- DEVONthink 4
- Mail.app (only needed when email import is required)
...
1. Clone the repository (prerequisite: [osagitfilter](https://github.com/doekman/osagitfilter)).
...
3. Create a [configuration](Docs/configuration.md) for the DEVONthink database you want to use.
4. Set up your workflow:
	...
	- Install the [PopClip](PopClip) extension
...
- Regression in AppleScript performance since DEVONthink 4.2 on macOS 26 Tahoe (see the issue description [here](https://discourse.devontechnologies.com/t/regression-in-applescript-performance-with-dt-4-2)). Two workarounds have been identified to mitigate the issue:
	...
	2. Running time-consuming DEVONthink menu scripts and worker-enabled smart rules via an external `osascript`, which reduces overall elapsed time (see [here](Docs/devonthink-menu-worker-pattern.md)).
