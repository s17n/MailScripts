...
-- Erstellt fŸr jeden Record (.eml) einen Kontakt und fŸgt diesen einer Kontaktgruppe hinzu 
	...
	tell application id "DNtp"
		...
		repeat with theRecord in theRecords
			...
			tell theRecord
				...
				set {theAuthorEmail, theAuthorName} Â
...
-- Importiert Mail Messages nach DEVONthink u. verschiebt sie anschlie§end in das Mailbox-Archiv.
-- 	theMessages : Die zu importierenden Messages.
--	theDatabase : DEVONthink Datenbank, in die importortiert wird.
...
on addMessagesToDevonthink(theMessages, theDatabase, theImportFolder, theMailboxAccount, theArchiveFolder)
	...
	tell application "Mail"
		repeat with theMessage in theMessages
			try
				tell theMessage
					set {theDateReceived, theDateSent, theSender, theSubject, theSource, theReadFlag} Â
...
-- FŸr eml basierend auf 'creation date'
-- FŸr alle anderen Typem basierend auf dem Custom Meta Data "Date"
